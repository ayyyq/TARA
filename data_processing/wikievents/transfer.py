...
def transfer(split, MAX_LEN=400):
    data = []
    new_data = []
    coref = {}
    ...
    for d in tqdm(data):
        ...
        for entity_mention in entity_mentions:
            entity_information[entity_mention['id']] = (entity_mention['start'], entity_mention['end'] - 1)
        coref_entities = coref[doc_id]

        all_triggers = [event_mention['trigger'] for event_mention in event_mentions]
        ...
        for i, event_mention in enumerate(event_mentions):
            ...
            cur_len = len(sentences[trigger_sent][:-1][0])
            context = [[tup[0] for tup in sentences[trigger_sent][:-1][0]]]
            if cur_len > MAX_LEN:
            ...
            new_d['other_evt_triggers'] = []
            for i_t, trg in enumerate(all_triggers):
                if i_t == i:
                    continue
                trg_b, trg_e = trg['start'] - offset, trg['end'] - 1 - offset
                if 0 <= trg_b < len(all_context) and 0 <= trg_e < len(all_context):
                    new_d['other_evt_triggers'].append([trg['start'] - offset, trg['end'] - 1 - offset])
            ...
            for argument in arguments:
                ...
                new_d['gold_evt_links'].append(
                    [[trigger_b - offset, trigger_e - offset], [span_b - offset, span_e - offset], role])

                assert trigger_b - offset >= 0 and trigger_b - offset < len(
                    all_context) and trigger_e - offset >= 0 and trigger_e - offset < len(all_context)
                assert tokens[trigger_b] == all_context[trigger_b - offset]
                assert tokens[trigger_e] == all_context[trigger_e - offset]

                if span_b - offset >= 0 and span_b - offset < len(
                        all_context) and span_e - offset >= 0 and span_e - offset < len(all_context):
            ...
            for coref_entity in coref_entities:
                ...
                for entity in coref_entity:
                    ...
                    if entity_span_b >= 0 and entity_span_b < len(
                            all_context) and entity_span_e >= 0 and entity_span_e < len(all_context):
