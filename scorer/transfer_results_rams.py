...
if 'test' in args.outfile_golden:
    for d in tqdm(golden_data):
        r = {}
        r['doc_key'] = d['doc_key']
        r['gold_evt_links'] = []
        ...
        for gold_evt_link in gold_evt_links:
            ...
            span_b, span_e = gold_evt_link[1]
            span_head = get_head(doc, span_b, span_e)
            r['gold_evt_links'].append([trigger_span, [span_b, span_e], role_name])
        golden_new_data.append(r)

    with open(args.outfile_golden.replace('head', 'span_identification'), 'w') as f:
        for d in golden_new_data:
            f.write(json.dumps(d) + '\n')
...
if 'test' in args.outfile_prediction:
    for d in tqdm(prediction_data):
        r = {}
        r['doc_key'] = d['doc_key']
        r['predictions'] = [[]]
        ...
        for pre in prediction[1:]:
            span_b, span_e = pre[0], pre[1]
            span_head = get_head(doc, span_b, span_e)
            r['predictions'][0].append([span_b, span_e, 'n/a', pre[3]])

        prediction_new_data.append(r)

    with open(args.outfile_prediction.replace('head', 'span_identification'), 'w') as f:
        for d in prediction_new_data:
            f.write(json.dumps(d) + '\n')