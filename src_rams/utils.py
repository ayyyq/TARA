...
def collate_fn(batch, vocab_pad):
    ...
    trigger_spans = torch.LongTensor([ex['trigger_span'] for ex in batch])
...
class RAMSCheckpointCallback(CheckpointCallback):
    def __init__(self, folder=None, topk=0, monitor=None, larger_better=True):
        super().__init__(folder=folder, topk=topk, monitor=monitor, larger_better=larger_better)
        self.monitor = monitor
        self.larger_better = larger_better

    def on_evaluate_end(self, trainer, results):
        if trainer.cur_epoch_idx < 40:
            def monitor(results):
                return 0.0
