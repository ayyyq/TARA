...
class OutputSaver_EVAL(Metric):
    ...
    def update(self, preds, labels, spans, all_selected_spans, event_ids, doc_keys, start_subwordidx2wordidx, end_subwordidx2wordidx):
        self.pred_list += preds.tolist()
        self.label_list += labels.tolist()
        self.span_list += spans.tolist()
        for selected_spans in all_selected_spans:
            self.selected_spans_list.append(selected_spans.tolist())
    ...
    def get_metric(self):
        ...
        if get_global_rank() == 0:
            with open(self.save_fp, 'w') as writer, open(self.ref_fp, 'r') as reader:
                for pred, label, span, selected_spans, event_id, doc_key, start_subwordidx2wordidx, end_subwordidx2wordidx, line in zip(
                        self.pred_list, self.label_list, self.span_list, self.selected_spans_list,
                        self.event_id_list, self.doc_key_list, self.start_subwordidx2wordidx_list,
                        self.end_subwordidx2wordidx_list, reader):
                    example = json.loads(line)
                    assert doc_key == example['doc_key']
                    ...
                    for selected_span in selected_spans:
                        new_selected_spans.append([start_subwordidx2wordidx[selected_span[0]], end_subwordidx2wordidx[selected_span[1]]])

                    result = {'doc_key': doc_key,
                              'predictions': [[]],
                              'selected_spans': new_selected_spans}
